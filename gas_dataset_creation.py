...
# Change directory, obtain the gas csv files and print the csv files 
os.chdir('C:/Users/bencr/Downloads/Dataset_1') 
csv_files = ['gas_usage_2023.csv', 'gas_usage_2024.csv'] 
print(csv_files) 
...
# Concatenate the datasets in the data frame, convert the data frame to a csv file, drop the “Values” column, convert the date to datetime and convert the updated data frame to a csv file for gas 
final_df = pd.concat(dfs, ignore_index=True) 
final_df.to_csv('energy_dataset4.csv', index=False) 
final_df.drop(labels=['Values'], axis=1, inplace=True) 
final_df['Date'] = pd.to_datetime(final_df['Date'], format="%d/%m/%Y") 
final_df.to_csv('gas_dataset2.csv', index=False) 
...
# Fill in the gaps of the final_df, convert the updated data frame to csv and print the filled gas dataset
final_df = gaps(final_df)
final_df.to_csv('gas_dataset2_filled.csv', index=False) 
print(final_df) 
